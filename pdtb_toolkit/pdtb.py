...
class CorpusReader:
    ...
    def iter_data(self, display_progress=True):
        row_iterator = csv.reader(open(self.src_filename))
        # row_iterator.next()  # Skip past the header.
        next(row_iterator)  # Eleanor.
        i = 1
        for row in row_iterator:
            if display_progress:
                sys.stderr.write("\r");
                sys.stderr.write("row %s" % i);
                sys.stderr.flush()
                i += 1
            yield Datum(row)
...
class Datum:
    header = [
        ...
        ##### Connective.
        'Connective_SpanList', 'Connective_GornList', 'Connective_Trees', 'Connective_RawText',
        'Connective_StringPosition',
        'SentenceNumber', 'ConnHead', 'Conn1', 'Conn2', 'ConnHeadSemClass1', 'ConnHeadSemClass2', 'Conn2SemClass1',
        'Conn2SemClass2',
    ...
    def __init__(self, row):
        ...
        for i in range(len(row)):  # Eleanor
