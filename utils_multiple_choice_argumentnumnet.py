...
class Split(Enum):
    train = "train"
    dev = "eval"  # for ReClor. Other datasets "dev".
    test = "test"
    ...
    class MyMultipleChoiceDataset(Dataset):
        ...
        def __init__(
            ...
            cached_features_file = os.path.join(
                data_dir,
                "cached_data",
                "argument_numnet_cached_{}_{}_{}_{}_dataprov{}_graphv{}{}".format(
                    mode.value,
            ...
            with FileLock(lock_path):
                ...
                if os.path.exists(cached_features_file) and not overwrite_cache:
                    ...
                    if mode == Split.dev:
                        ...
                        # examples = processor.get_train_examples_w_arg_dom_words(data_dir, argument_dir)
                        if demo:
...
def convert_examples_to_arg_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            ...
            if isinstance(arg_tokenizer, tuple):  # for try_data_34.py
                arg_tokenizer, substitution_flag = arg_tokenizer
                inputs = arg_tokenizer(text_a, text_b, tokenizer, stopwords, relations, punctuations, max_ngram,
                                       max_length, substitution_flag)
            else:
        ...
        if isinstance(argument_bpe_ids[0], tuple):  # (argument_bpe_pattern_ids, argument_bpe_type_ids)
            # list(len=n_choices) of (list, list) -> (list[list], list[list])
            arg_bpe_pattern_ids, arg_bpe_type_ids = [], []
            for choice_pattern, choice_type in argument_bpe_ids:  # (list, list)
